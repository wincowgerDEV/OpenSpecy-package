...
- **Dependencies**: no new R dependency. Use the pinned webR 0.6.0 `WebR.FS.mount("WORKERFS", {files}, mountpoint)` capability and browser `File` structured cloning. A deterministic adapter against the pinned Shinylive asset is allowed only after an exact-artifact prototype; fail closed on an asset/hash mismatch rather than editing generated output by hand.
...
- **Pipeline diagram**: `.specify/memory/pipeline-diagram.html` is updated with this plan to show the target explicitly: local temporary files and hosted WORKERFS mounts converge at “Read & Materialize”; remove “Memory preflight safe?”/“Analysis Blocked”; add “Active Spectrum Inspection” and “Quality Findings”; route Heatmap selection through that inspection lane; retain dataset “Signal/Noise Compute” for thresholding/histogram only. Remove the target-status note when implementation and source locations agree.
...
- [ ] Prototype WORKERFS mounting in an ignored export built from the exact pinned Shinylive/webR artifact; prove browser `File` bytes bypass the multipart/httpuv/R-raw bridge before changing app source.
- [ ] Baseline the genuine ENVI ZIP, its direct HDR+DAT pair, and a small synthetic equivalent by mount/read/materialize/analysis phase; retain ignored hashes, timing, and peak/copy evidence.
- [ ] Remove memory prediction/gating and replace its tests/UI/diagram branch with actual-failure guidance (`R/app_memory.R`, `inst/shiny/`, `tests/testthat/`).
- [ ] Implement the single-spectrum inspection/QC lane and collapsed rejected-pixel regression (`inst/shiny/{global.R,server.R}`, app tests/smoke).
- [ ] Apply 2420-2550 cm^-1 and 2x package/app defaults; update focused scientific tests, roxygen, SOP, and NEWS.
- [ ] Implement the session-scoped hosted mount handshake and deterministic pinned-asset adapter; converge on ordinary `read_any()` and a fully in-memory `OpenSpecy`, with cleanup and small-file fallback.
- [ ] Reconcile the already-updated target diagram with final function names/source locations, remove its target-status note, then run focused, benchmark, package, app-browser, and hosted tiers once.
