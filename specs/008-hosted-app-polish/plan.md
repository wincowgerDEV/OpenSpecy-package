...
- R10. The app defaults isolated-spike correction on and saturated-range removal off, gates child controls, places those less-common boxes last, and processes raw upload -> spike correction -> optional saturation restriction -> ordinary preprocessing/current CO2/high-tail stages -> identical library-axis restriction -> identification. Rejected saturation stays visible and is exported in processing metadata.
- R11. Three always-visible controls above the active spectrum show automatic corrections, warnings, and successes. Automatic lists spike, saturation, CO2 flattening, and high-tail outcomes from the correction state that produced the displayed spectrum, counts only applied corrections, and names corrected/retained ranges; those checks are excluded from warning/success. Warning and success modals contain only their exact failed/passed results, including selected-spectrum SNR and correlation relative to app thresholds; success findings are descriptive and omit empty interpretation/action fields. Use rainbow automatic, yellow warning, and green success icon/outlines.
- R12. Every hosted download choice produces its intended suggested filename, MIME/disposition, nonempty bytes, and format signature through a real user click; neither a blank HTML fallback nor a canceled native download may pass because an in-frame `fetch()` succeeds. Top Matches must select Top N from the compact correlation matrix before metadata joins, preserve legacy Simple/All values/schema/tie order, remain bounded for the 208-spectrum test map, and leave the session healthy for later downloads.
...
- R14. Move plot legends outside data regions; numeric heatmaps show a horizontal, non-overlapping legend above the plot, categorical heatmaps may omit it. Resolve the default map color server-side so Match Value never flashes before default Match Name. Reuse the exact Match Name category-to-color mapping in the material-class summary and retain the bright colorblind-accessible palette.
...
- R17. Export a pipe-friendly `point_intensity()` for one nearest or linearly interpolated wavenumber; keep single-region measurement in `area_under_band()`. The app adds Measurements after Custom Ratios, treats both builders as active whenever their single Quantification owner is on, uses numeric inputs rather than sliders, and exports definitions/values from the displayed processed spectra.
...
- R19. Every non-obvious processing box provides adjacent "What this changes" help naming each input, units/scale, how modes or higher/lower values affect analysis, and important rejection/no-op/interpretation consequences; spike and saturation help satisfy this contract explicitly.
- R20. The contextual download action fills the usable width of its card header without obscuring the collapse control at desktop or mobile widths.
...
- **Download diagnosis**: The parent-frame fetch/Blob bridge is sound. The current public 208-spectrum map returned HTTP 500 because Top Matches melted 968,240 correlations and joined roughly 190 columns before ranking, then WebR failed with `cannot allocate vector of size 3.6 Mb`. Rank per query first, enrich at most Top N candidates, and probe a one-row metadata schema so rare legacy All columns remain present without the memory-heavy full join.
...
- **Clean wasm source**: Build Docker `local::.` from a read-only `git archive` of the exact candidate SHA and mount repository/image outputs separately, so ignored rehearsal scratch cannot recursively enter the OpenSpecy source archive.
...
- `benchmarks/`: repeated reference-vs-current correction, assessment, point lookup/ratio, default single/100-spectrum/map app-stage cases, and legacy-vs-ranked Top Matches exports; require identical CSV bytes and flag >10% same-output regressions.
...
- [x] Reproduce the public test-map HTTP 500, rank Top Matches before metadata expansion, preserve exact export behavior, add the repeated benchmark/unit contracts, and pass a fresh-session local 208-spectrum map plus follow-up download.
- [x] Build the clean-candidate Docker wasm artifact, then pass the full pre-push wrapper and map Top Matches plus follow-up Processed Spectra native-download regression against that exact SHA; retain the JSON, manifest, site, and browser evidence.
- [x] Verify a repeated pre-push run stages only tracked candidate source (no `_wasm` recursion), keeps the OpenSpecy source archive bounded, and writes the exact-SHA JSON report.
- [x] Remove the default heatmap metric flash, move the numeric legend above the map, simplify quantification ownership, reorder/expand processing controls, refine success/automatic details, default saturation off, and make the download action full-width.
- [x] Accept bounded monotonic spike-correction passes without weakening safeguards; record canonical correction state/ranges and add focused plus browser regressions for all minor UI refinements.
...
- Apply `openspecy-develop-shiny-app`: exercise no-upload, raw/processed, identified, batch/map, quantification, correction accepted/rejected, all downloads, progress, console, screenshots, and asset inventory. For Top Matches, compare legacy/current Simple and All CSV bytes, enforce one row per map spectrum at Top N = 1, a 15-second local endpoint ceiling, no allocation error, and a successful follow-up download.
...
- Minor app refinement approved by: user, 2026-08-04 (`speckit-implement`; heatmap, quantification, processing help/order, diagnostics, correction state, and download layout).
...
- Download-fix evidence: clean candidate `f4c5e46` passed the full Docker pre-push gate on 2026-08-03. It built the exact 116-package closure from a 17,307,985-byte tracked-source snapshot with no `_wasm` tree (7,107,999-byte OpenSpecy source archive), passed all four library probes, produced a 184,027,573-byte site, and passed the 2.0-minute click-to-disk browser smoke including 208-row Top Matches and follow-up map Processed Spectra; retained JSON records `status = passed`.
- Minor-refinement evidence: focused 105-spike and 529-app suites plus all 1,967 package assertions passed; both genuine-file browser tests passed in 1.7 minutes; the 208-spectrum correction median was 3.42 seconds; and a 17,335,109-byte staged source passed `R CMD check` with only the existing UTF-8 data note.
- Follow-up: have the maintainer commit and push, then verify the matching public wasm artifact and Pages publication; these are the only post-push checks still pending.
