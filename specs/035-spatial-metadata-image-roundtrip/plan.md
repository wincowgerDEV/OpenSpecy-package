...
- R7. The registered image renders above the opaque classified/rejected heatmap, and Overlay Transparency controls only that image. Dominant separated red-frame lines define the ENVI bounds and align with the heatmap cell boundary; red annotations do not expand the crop. Selection stays above the image. Toggle/transparency changes are live and do not recompute science.
...
- R12. Per-source files written by `automate_particle_analysis()` append the complete input basename (without its extension). One-region split H5 names are not truncated to `Region*`; multi-region sources append the region after the full source basename. Aggregate `*_all.csv` names remain unchanged.
- R13. Each `automate_particle_analysis()` material-summary row retains its particle count and adds the full map-region area plus total, mean, and median particle area in square micrometres, using the same pixel-length calibration as particle details. Full map area includes rejected pixels; empty summaries retain map area, report zero total particle area, and use missing mean/median particle area.
