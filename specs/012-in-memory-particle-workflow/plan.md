...
**Current tranche**: Remove app FileSpecs routing and rebuild upload, reduction, matching, and display as one in-memory workflow; retain package FileSpecs.  
**Change class**: Mixed; highest hosted/release, plus package/scientific and bundled-app behavior.  
**Status**: Implemented; hosted smoke awaits a matching action-built artifact.
...
- R5. One final `OpenSpecy` reactive feeds spectra, summaries, identification, quantification, metadata, downloads, and selection. A pixel-to-unit table only projects final IDs/scores/masks/clicks to the map.
...
- R7. Cluster strategies subdivide eligible pixels globally or within connected threshold/material regions. Fit one shared spatial-only PCA; UI components/K start at 10, clamp/show effective limits, seed locally, filter size after grouping, and preserve result outputs. Geometric mean requires positive values.
...
- R10. Omit plot/colorbar titles; use about one-quarter current colorbar dimensions (`len ~= 0.18`, `thickness ~= 4`) and reclaim top margin. Always show themed S/N, and use the same interactive histogram renderer in all states; draw only enabled threshold lines.
...
- **Correlation collapse**: Fully process S/N-eligible pixels, identify once, retain top N, and project best score/material to pixels. Filter thresholds, then connected mode groups adjacent equal materials; within-region/global cluster modes subdivide those material groups. Collapse source spectra and reuse identities/top N; never correlate collapsed units again.
...
- **Local browser**: Use `CA_tiny_map.zip` and optional larger H5; test S/N-off/correlation-on, three collapse modes, masks, legend, histograms, click sync, memory recovery, progress, console, and screenshots.
