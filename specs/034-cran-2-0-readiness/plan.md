...
- R21. Let `automate_particle_analysis()` explicitly choose bounded `file_processing = "stream"` (default) or full `"memory"` materialization; support streamed `all_cell_id` matching and connected Mean collapse, with chunk/spectrum/percent/elapsed console progress.
...
- [x] Expose stream/memory particle execution, add bounded chunk progress and parity tests/benchmark/docs, and preserve the prior 4.94 GB/2,384-spectrum bounded-allocation evidence.
...
- Current R 4.3.3 candidate: full tests pass 3,984 assertions with two guarded AWS skips, focused particle tests pass 286 assertions, and hosted-static passes 361 assertions. The staged check rebuilds both CRAN vignettes with 0 errors, 0 warnings, and one explained valid-UTF-8 data NOTE in 11.8m. R 4.6.1 remains to be rerun after the current source changes; current CRAN reverse dependencies: none.
- The exact `41722b5` GitHub wasm artifact passes its 118-package closure, staged AWS library match (0.9715), 269 MB site assembly, and full desktop/mobile browser workflow after selecting the typed Raman medoid partition and accepting the 2.0 library label/quoted CSV contract. The `858e068` CI run stopped before building when one Docker Linux-engine probe exceeded 10 seconds; bounded preflight retries now cover runner startup races. A clean post-fix CI rerun and R-patched/devel Linux/macOS checks remain, so the last two checklist items stay open.
