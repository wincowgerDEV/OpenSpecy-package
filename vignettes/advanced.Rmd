...
# Compact In-Memory Spectra

For advanced map and repeated-spectrum work, `Specs` can retain spectra in
memory while compacting regular coordinates and repeated metadata. Dense
`OpenSpecy` remains the default for `read_envi()` and `read_h5()`; request
`representation = "Specs"` explicitly. An optional background policy is lossy:
it maps rejected or non-finite pixels to an exact-zero virtual spectrum and
records that decision. Coordinate and repeated-metadata compaction is lossless.

```{r compact-specs, eval=FALSE}
policy <- specs_background_filter(
  metric = "run_sig_over_noise", minimum = 4, sigma = c(1, 1, 1)
)
map <- read_envi(
  "map.dat", "map.hdr", representation = "Specs",
...
transformations. Background spectra keep code or class zero and are excluded
from fitting. Use one fitted PCA/Hilbert model for both reference and query
objects so their codes share the same limits.

```{r encoded-specs, eval=FALSE}
model <- fit_specs_pca(spec_lib, n_components = 16)
library_specs <- as_Specs(spec_lib, model)
query_specs <- as_Specs(
  raman_proc, model, limits = attr(library_specs, "hilbert_model")
)
match_spec(query_specs, library_specs, top_n = 5)
```

For a large in-memory `OpenSpecy` query, bound the temporary
reference-by-query correlation matrix with `batch_size`. A smaller positive
integer lowers peak memory and adds loop overhead; a larger value is faster
when memory permits. Bounded matching requires a finite `top_n` and preserves
the same ranked rows and ordering as the dense calculation.

```{r bounded-match, eval=FALSE}
matches <- match_spec(
  query_map, library = spec_lib, top_n = 1, batch_size = 1000
)
```

The file-backed `FileSpecs` workflow for maps larger than memory is documented
in the standard operating procedure. The bundled and browser app pipeline,
including spatial and spectral clustering behavior, is documented in the app
vignette.
...
[NIST wavenumber formulation](https://www.nist.gov/publications/self-study-manual-optical-radiation-measurements-part-i-concepts-chapter-12).
...
910-8.
[NIST publication record](https://www.nist.gov/publications/self-study-manual-optical-radiation-measurements-part-i-concepts-chapter-12).
