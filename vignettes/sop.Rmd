...
tutorial](https://www.openanalysis.org/OpenSpecyV2/pkgdown/articles/app.html).
...
[the embedded browser app](https://www.openanalysis.org/OpenSpecyV2/) or
...
Open Specy allows for upload of native Open Specy .csv, .json, or .rds files. Open Specy and .csv files should always load correctly
...
Tutorial](https://www.openanalysis.org/OpenSpecyV2/pkgdown/articles/spectragryph.html). Unfortunately the maintainer of Spectragryph passed away and it is unclear how much longer this will be supported.

## Experimental large H5 and ENVI workflow

Use `FileSpecs` for a hyperspectral map that is too large for an ordinary
in-memory `OpenSpecy` workflow. It is a lightweight, local-first `Specs`
subtype: the serialized object contains source, axis, coordinate, recipe,
fingerprint, and cache descriptors, but no open connection and no full values
matrix. `open_specs()` currently accepts H5 or an ENVI `.hdr` plus `.dat`/`.img`
pair. It opens each member read-only and never writes beside or over the source.

Choose a dedicated cache directory. Completed cache entries are versioned by
the source fingerprint and published atomically. A changed, missing, or moved
source is rejected instead of silently reusing stale results. A saved
descriptor therefore still needs its original, unchanged source files. Remove
only a cache directory that you created, and only after every session using it
has ended; source files are not cache files.

```{r, eval=FALSE}
cache_dir <- file.path(tempdir(), "openspecy-large-map-cache")
map <- open_specs("D:/spectral-data/map.h5", cache_dir = cache_dir)
map

# Natively copy one region at a time as map_Region1.h5, etc. Spectral values
# are not loaded into R.
region_files <- split_h5(
  "D:/spectral-data/map.h5",
  output_dir = "D:/spectral-data/regions"
)

# These are descriptor-only regional views.
region_views <- split_spec(map, by = "region")

# Materialize only a reviewed selection. A whole-source request is refused.
pixel <- decompress_spec(region_views[[1]], index = 1)
roi <- decompress_spec(map, region = "Region1",
                       roi = c(xmin = 10, xmax = 30,
                               ymin = 20, ymax = 40))

# Save the descriptor, not a copy of the spectral cube.
write_specs(map, "map.filespec.rds")
map <- read_specs("map.filespec.rds")

# Or stream one complete rectangular region to a new float64 ENVI pair.
# Existing output members are refused rather than overwritten.
write_spec(region_views[[1]], "region-1-export.hdr")
```

`split_h5()` defaults to `field = "region"` and native H5 output. Another
indexed or H5 metadata field can be used when each category contains complete
regions. For a field that divides pixels within a region, use `format = "rds"`;
each materialized category must then fit in memory. The function returns output
paths invisibly, sanitizes invalid filename characters, and refuses to
overwrite existing files. Native splits retain the registered H5 mosaic tiles
that intersect their selected regions, with matching `Centers` rows and
renumbered image datasets, so `visual_image()` and particle overlays continue
to work on each output.

The export preserves the wavelength axis with round-trip-safe formatting and
rejects custom writers, incomplete/multi-region views, source members, and
existing output paths.

Selecting an entire region is explicit but may still be large; prefer an index
or a small region of interest before calling established `OpenSpecy` functions.
Matrix-only `Specs` operations fail with guidance rather than falling back to a
hidden whole-cube read.

The first supported direct analysis is region-aware particle identification
with S/N screening and exact mean collapse. It streams each region, rereads
only retained pixels to form particle means, materializes that small collapsed
object, and then uses established preprocessing and exact matching. Thresholds
remain scientific choices: `sn_threshold_min` and `sn_threshold_max` bound the
accepted S/N scale, `area_threshold` rejects smaller connected regions in
pixels, and `cor_threshold` is the minimum correlation used for interpretation.

```{r, eval=FALSE}
particle_result <- automate_particle_analysis(
  map,
  library = reference_library,
  file_processing = "stream",
...
# Recorded plots survive after the pipeline returns.
plot(particle_result, sample = "Region1", which = "particle_heatmap")
plot(particle_result, sample = "Region1", which = "sn_histogram")
plot(particle_result, sample = "Region1", which = "cor_histogram")
```

For a `FileSpecs` object or H5/HDF5 path, `file_processing = "stream"` is the
default. It reports region, chunk, spectrum-count, percentage, and elapsed-time
milestones while reading bounded blocks. Choose `file_processing = "memory"`
to materialize every region before analysis when the full spectra and analysis
intermediates fit comfortably in RAM. Memory mode enables the complete
in-memory strategy set and can be faster for smaller maps, but it removes the
streaming memory bound.

The signal heatmap uses a continuous finite minimum-to-maximum legend; the
threshold mask remains binary. Both histograms draw the configured threshold
lines. The pipeline retains the exact best match on each collapsed particle.
For H5 data, requesting
`particle_image` materializes and caches the registered visual mosaic only for
the current region, using all intersecting tiles rather than one unrelated
tile. Stream mode supports connected `collapse` with `mean` and
`all_cell_id`, non-entropy S/N, and bounded spectral smoothing. Other
strategies, median/custom collapse, and arbitrary whole-map operation chains
require memory mode. Verify stream/memory equivalence on a representative
subset before interpreting a large run.

For an ENVI DAT/IMG path, `automate_particle_analysis()` also looks beside the
spectral pair for one same-basename JPG, JPEG, or PNG when `images` is omitted.
It uses the red-boundary detector to register that image and draws the same
material-class overlay as `particle_image()`. Multiple matches or an
undetectable boundary warn and leave the spectral analysis usable without an
image.

Local Shiny uses one **Choose spectra...** control: a native Windows/macOS
multi-file dialog when available, or a mounted-filesystem browser elsewhere.
It keeps the original paths instead of
uploading duplicate files. Shinylive mounts browser-selected files through
WORKERFS. Its 10 GiB input ceiling is a transport limit rather than a promise
that every operation will fit in RAM. ENVI/H5 maps normally use compact
file-backed coordinates and metadata; **Load Entire File into Memory** is an
explicit advanced override, with a configurable identification batch size.
Optional background suppression maps rejected pixels to virtual zero while
retaining per-source S/N and reasons. Selection only stages a source; **Run**
owns the read and every later operation.

For an uploaded map, the app applies optional spatial smoothing before any
particle decision. Signal/noise is calculated from that spatial-only object.
Connected regions collapse eligible pixels before ordinary processing and
identification. Both clustering strategies instead fit one source-scoped PCA
plus K-means model to the spatial-only spectra, collapse those spectral groups,
and process and identify the clusters once. Non-spatial mode returns those
clusters. Spatial mode projects the cluster material identities to the original
pixels, joins adjacent equal-material clusters, collapses the raw/spatial-only
spectra again, and reprocesses the final particles without identifying twice.
When correlation is a threshold, that same first Top N identification result is
reused. Threshold failures are background pixels, not unknown particles.

The default **Mean Up** conformation technique leaves the uploaded axis
unchanged and conforms the identification library onto it with memory-bounded
averaging and interpolation, unless the selected Wavenumber Resolution is
finer than the upload's native resolution, in which case the uploaded spectra
are resampled up to that resolution instead. Heatmap legends open from **View Legend**; categorical
maps with more than 30 values rely on hover instead of listing an unreadable
legend. A rejected pixel has no nearest-particle substitution: its processed
trace is flat and it has no matches. Thresholded-particle downloads can include
the final summary table, both histograms, all available heatmaps, the material
summary, and particle-size distribution. The package-only experimental workflow
above remains available independently for bounded on-disk research.

Maintainers can run the repeated benchmark in
`benchmarks/file_specs_particle_analysis.R`. It is opt-in and accepts external
H5, ENVI, eager-RDS oracle, and reference-library paths through environment
variables; it never copies the large source into the repository.
...
Open Specy objects can be saved most accurately as .csv, .rds, or .json files.
.rds will be the most reproducible as it is a native R file format and floating
point errors can happen with .csv or .json.
...
Isolated impulse artifacts and detector saturation should be handled before
ordinary smoothing, baseline correction, derivatives, or normalization. The
package keeps both operations opt-in; the app exposes them as owner controls and
uses this order when they are enabled:

```{r eval=FALSE}
quality_ready <- raman_hdpe |>
  correct_spike(method = "residual", direction = "both") |>
  restrict_range(saturation = "auto", make_rel = FALSE) |>
  process_spec(correct_spike = FALSE)
```

The default residual method is deliberately limited to isolated points and
uses the actual wavenumber coordinates for its local interpolation. For Raman
work, `correct_spike()` also implements the prominence/FWHM procedures described
by [Coca-Lopez (2024)](https://doi.org/10.1016/j.aca.2024.342312). The manual
paper mode requires prominence and width thresholds calibrated for the
material, instrument, resolution, and acquisition conditions. Its automated
prominence/FWHM-ratio mode uses an upper outlier threshold of 3.5 standard
deviations only when enough peaks are available. Width is measured in sampled
CCD-pixel units, not cm^-1^; the paper reference correction flags intervals at
`rel_height = 0.8` and interpolates from `m = 10` unflagged points on each side.

No single-spectrum detector can always distinguish a cosmic-ray spike from a
genuine narrow band. Inspect correction diagnostics, calibrate on representative
standards, and pay particular attention to spikes on real bands and to
narrow-band materials such as calcite and polystyrene. Boundary candidates are
not extrapolated. If a feature is ambiguous, retain the raw data and recollect
rather than treating an interpolated peak as measured evidence.

`saturation = "auto"` detects a hard maximum plateau only when it has lower
shoulders; a known finite detector ceiling can be supplied instead. Detected
intervals are unioned across the batch, guard-expanded, and removed from every
spectrum on the same shared axis. At most 70% of wavenumber-cell coverage is
removed by default. A proposal over that inclusive limit, or one leaving too
few points, is rejected with a warning and unchanged spectral values. Apply the
identical retained axis to the reference library before identification. Open
Specy does not reconstruct saturated peaks: recollection at lower exposure,
laser power, or integration time is the scientifically preferred remedy.

Processed spectra can be checked and corrected before identification. The
default CO2 and high-tail checks compare the artifact-region maximum with the
maximum outside both artifact regions after temporary min-max normalization.
The default artifact ratio is 2: a candidate is flagged at or above twice the
control-region maximum. Raising this dimensionless ratio makes detection and
automatic correction less sensitive; an explicit value remains authoritative.
The default silent-region quality check covers 2420--2550 cm^-1^ and is applied
to each assessed spectrum, not as a blanket dataset result.
With `automate = TRUE`, each range function returns spectra unchanged when its
issue is absent. Otherwise, `flatten_range()` flattens a flagged CO2 region and
`restrict_range()` crops a shared batch tail only when no more than 20% of the
full wavenumber span must be removed.

```{r eval=FALSE}
assessment <- assess_spec(
  processed,
  checks = c("co2_region", "high_tail")
)
corrected <- processed |>
  flatten_range(automate = TRUE, make_rel = FALSE) |>
  restrict_range(automate = TRUE, make_rel = FALSE)
```

The compatibility default returns issue rows only and stays empty when a clean
spectrum has no findings. Use the complete report for auditable UI or batch
status, and opt into spike and saturation checks explicitly. The exact
breakpoint SNR sorts amplitudes and evaluates every possible two-group split;
it does not depend on histogram bins.

```{r eval=FALSE}
quality_report <- assess_spec(
  quality_ready,
  checks = c("missing_values", "negative_intensity", "low_snr",
             "spike", "saturation"),
  report = "all",
  snr_metric = "breakpoint_snr"
)
```
...
spectral_map_p <- spectral_map |>
  process_spec(flatten_range = TRUE)
...
p1 <- smooth_intens(raman_hdpe, polynomial = 1, derivative = 0, abs = FALSE)
p4 <- smooth_intens(raman_hdpe, polynomial = 4, derivative = 0, abs = FALSE)
...
d1 <- smooth_intens(raman_hdpe, derivative = 1, window = window, abs = TRUE)
d2 <- smooth_intens(raman_hdpe, derivative = 2, window = window, abs = TRUE)
...
zero absorbance. Open Specy provides modified polynomial fitting and 4S Fill
Peaks (Liland 2015) as automated methods. Fill Peaks uses iterative local-window
suppression and is useful for nonlinear baselines with local variation. The
modified polynomial method iteratively fits polynomial equations to the whole
spectrum; a higher polynomial order produces a more flexible fit but can remove
peaks when the baseline is not very wiggly. Manual baseline correction can also
be specified by providing a baseline `OpenSpecy` object. There are many fine
tuning options that can be chosen, see `?subtr_baseline()` for more details.
...
alternative_baseline <- smooth_intens(raman_hdpe, polynomial = 1, window = 51,
                                      ...
                                      make_rel = FALSE) |>
  flatten_range(min = 2700, max = 3200, make_rel = FALSE) #Manual baseline with heavily smoothed spectra
...
dr <- subtr_baseline(raman_hdpe, refit_at_end = TRUE) #optionally retain baseline noise with refitting

c_spec(list(none, d, d8, dr)) |> 
  plot(offset = 0.25)
```

For example, apply Fill Peaks to the FTIR LDPE spectrum loaded earlier while
retaining its intensity scale for later quantification.

```{r}
fill_peaks <- subtr_baseline(
  asp_example,
  type = "fill_peaks", lambda = 4, hwi = 50, it = 10,
  make_rel = FALSE
)
```
...
r1 <- restrict_range(raman_hdpe, min = 1000, max = 2000) |>
    conform_spec(range = none$wavenumber, res = NULL, allow_na = TRUE)
...
## Area and Point Measurements and Ratios

For a non-ratio measurement, `area_under_band()` returns the inclusive summed
intensity within one explicit region, while `point_intensity()` returns the
intensity at one requested wavenumber for every spectrum. The point method uses
the nearest measured value by default; `method = "linear"` interpolates between
adjacent measured points without extrapolating.

```{r}
single_region_area <- area_under_band(
  fill_peaks, min = 2700, max = 3000
)
single_point_intensity <- point_intensity(
  fill_peaks, wavenumber = 2880
)
single_region_area
single_point_intensity
```

For a custom area-over-area ratio, choose the numerator and denominator ranges
explicitly, calculate each with `area_under_band()`, and divide the returned
vectors. Bounds are inclusive, and each returned value stays aligned with the
corresponding spectrum column. The example below uses arbitrary teaching ranges;
select scientifically appropriate bands and preprocessing for your application.

```{r}
custom_area_numerator <- area_under_band(
  fill_peaks, min = 2700, max = 3000
)
custom_area_denominator <- area_under_band(
  fill_peaks, min = 1400, max = 1500
)
custom_area_ratio <- custom_area_numerator / custom_area_denominator
custom_area_ratio[!is.finite(custom_area_ratio)] <- NA_real_
custom_area_ratio
```

Named area-under-band ratios select sourced numerator and denominator bands;
they do not apply baseline correction, normalization, or another preprocessing
step. Apply the desired treatment first, then calculate the index. For example,
the specified area-under-band carbonyl index for PE and PP can be calculated
from the Fill Peaks result created above (Almond et al. 2020).

```{r}
saub_ci <- area_under_band(
  fill_peaks,
...
Additional area-over-area presets are `carbonyl_index_pe`,
`hydroxyl_index_pe`, `hydroxyl_index_pp`, `carbon_oxygen_index_pe`, and
`carbon_oxygen_index_pp`. See `?area_under_band` for their sourced band
definitions and intended materials. Baseline treatment and other preprocessing
can materially change an index value, so compare spectra only when the same
treatment was used and keep that treatment with the resulting metadata.

For point ratios, `peak_ratio()` divides the intensity at a numerator
wavenumber by the intensity at a denominator wavenumber for every spectrum.
The default `method = "nearest"` uses measured points and resolves an exact
midpoint tie toward the lower wavenumber. Set `method = "linear"` to interpolate
only between the adjacent measured points. Neither method extrapolates beyond
the available wavenumber axis, and the function does not search for a local
maximum.

```{r}
nearest_point_ratio <- peak_ratio(
  fill_peaks, numerator = 2880, denominator = 2840
)
linear_point_ratio <- peak_ratio(
  fill_peaks, numerator = 2880, denominator = 2840, method = "linear"
)
nearest_point_ratio
linear_point_ratio
```
...
now has more than 59,000 spectra and is too large to bundle in the R package. We
distribute the runtime libraries through AWS and provide `get_lib()` to download
them automatically. Running `get_lib()` by itself downloads every supported
runtime library to your package directory, or you can specify which libraries
...
processed <- process_spec(x = raman_hdpe, 
                          conform_spec = FALSE, #We will conform during matching.
                          smooth_intens = TRUE #Conducts the default derivative transformation.
                          )
...
many matches you want. Set `top_n_by` to one nonblank library metadata column,
such as `"organization"`, to retain that many matches independently from every
group. In this example we just identified a single spectrum with
...
```{r, eval=FALSE}
matches <- match_spec(x = processed, library = test_lib, conform = TRUE,
                      add_library_metadata = "sample_name", top_n = 1,
                      top_n_by = "organization")[order(match_val, decreasing = TRUE)]
...
```{r, eval=FALSE}
get_metadata(x = test_lib, logic = matches[[1,"library_id"]], rm_empty = TRUE)
```
...
Almond J, Sugumaar P, Wenzel MN, Hill G, Wallis C (2020). “Determination of the
Carbonyl Index of Polyethylene and Polypropylene Using Specified Area Under Band
Methodology with ATR-FTIR Spectroscopy.” *e-Polymers*, **20**, 369–381. doi:
[10.1515/epoly-2020-0041](https://doi.org/10.1515/epoly-2020-0041).
...
Suppression.” *MethodsX*, **2**, 135–140. doi:
[10.1016/j.mex.2015.02.009](https://doi.org/10.1016/j.mex.2015.02.009).
