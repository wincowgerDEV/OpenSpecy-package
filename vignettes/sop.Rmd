...
The Shiny app deliberately uses a separate in-memory workflow. Local Shiny
reads selected filesystem paths directly; Shinylive mounts the browser-selected
file through WORKERFS. Its 10 GiB input ceiling is a transport limit rather than
a promise that every operation will fit in RAM. ENVI/H5 maps use compact
coordinates and metadata, and optional configured background suppression maps
rejected pixels to virtual zero while retaining per-source S/N and reasons.
Selection only stages a source; **Run** owns the read and every later operation.
