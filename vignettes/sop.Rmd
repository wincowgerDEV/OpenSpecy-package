...
lines. With an in-memory `OpenSpecy` reference library, `top_n` can retain
multiple exact ranked names and values on each collapsed particle; compressed
library inputs currently retain only the top match. For H5 data, requesting
...
In the locally launched Shiny app, expand **Large local H5 / ENVI source**,
enter a server-side path, and click **Open read-only**. The app indexes the map,
renders a bounded region preview, lets you brush an exact ROI and pan/reset the
viewport, and materializes one selected spectrum for the existing controls and
downloads, avoiding a browser copy. Closing the source clears session state
without changing the source or completed cache generations. Ordinary uploads
are limited to 512 MB. The hosted WebAssembly app is limited to 100 MB and
cannot open a path on your computer; run `run_app()` locally for larger sources.
The path opener is enabled automatically by `run_app()`, disabled by default in
other Shiny deployments, and never enabled in WebAssembly. The large-map path
is not a promise that every app control can process a whole cube.
