...
author: >
  Jessica Meyers, Jeremy Conkle, Win Cowger, Andrew Gray, Chelsea Rochman
  Sebastian Primpke, Jennifer Lynch, Hannah Hapich, Hannah De Frond, Keenan
  Munno, Bridget O’Donnell
date: "`r Sys.Date()`"
...
Open Specy is a tool for viewing, processing, sharing, and identifying
Raman and (FT)IR spectra. This document outlines a common workflow for
...
spectra at an average speed of 1 per minute.
...
To get started with Open Specy, access
...
<!-- ![](images/image3.png){style="width: 100%"} -->
...
data into an Open Specy accepted format.

## Choose whether to share your uploaded data or not

<!-- [![](images/image3.png)]{style="width: 241.72px"} -->

[efore uploading, indicate if you would like to share the
uploaded data or not using the drop down. If "Share" is selected,
any data uploaded to the tool will automatically be saved in an open
access drive with a CC-BY 4.0 license and will be available for
...
they need to keep their data private. If "Not Now" is selected, none of
...
Open Specy allows for upload of .csv, .jdx, .0, .spc, .spa, and other
file types. .csv files will always load correctly but the other file
types are still in beta development, though most of the time these files
work perfectly. It is best practice to cross check non-.csv files in the
...
[[Spectragryph](https://www.google.com/url?q=https://www.effemm2.de/spectragryph/&sa=D&source=editors&ust=1616072638052000&usg=AOvVaw2H-GJzZfEy7EZ79blYTXgR)], which supports many spectral file conversions (see Mini Tutorial
section: File conversion in Spectragryph to Open Specy accepted
format).

If uploading a csv file, label the column with the wavelengths
"Wavelength" and name the column with the intensities "Absorbance".
Wavelength units must be 1/cm. Any other columns are not used by the
software. If you are uploading a reflectance or transmission spectrum,
you will still label the intensity units as Absorbance. Always keep a
copy of the original file before alteration to preserve metadata
...
time you can only upload one file at a time. Upon upload and throughout
the analysis, intensity values are min-max normalized (Equation 1).

Equation 1: Max-Min Normalization <!-- ![](images/image1.png) -->

Viewing Spectra Plot

<!-- [![](images/image3.png)]{style="width: 415.00px"} -->

After data is uploaded, it will appear in the main window. This plot is
selectable, zoomable, and provides information on hover. You can also
save a png file of the plot view using the camera icon at the top when
you hover over the plot. This plot will change the view based on updates
from the Adjusting Intensity Units selection.

## Adjusting Intensity Units 
...
adjustment uses the log10(1/T) calculation which does not correct for
system or particle characteristics. The reflectance adjustment
use s the Kubelka-Munk equation (1-R)2]/(2\*R). If none is selected, Open Specy
...
Mandatory inputs are marked with a red star. If these inputs are not
...
and metadata is sent to an [open-access online
repository](https://www.google.com/url?q=https://www.dropbox.com/sh/vibzoiq1i4o5usp/AAAe50RJhnvt6NoMDAJZ1CY8a?dl%3D0&sa=D&source=editors&ust=1616072638058000&usg=AOvVaw10FaY1OVqJbCMuruQQhErv). All inputs from the metadata (described below) are input to a
metadata sheet followed by any inputs on the smoother and baseline
correction page and ending with the date stamp. The metadata sheet is
...
high intensities and nonpeak regions should have low intensities.
...
(SG)
filter ([[https://en.wikipedia.org/wiki/Savitzky%E2%80%93Golay_filter](https://www.google.com/url?q=https://en.wikipedia.org/wiki/Savitzky%25E2%2580%2593Golay_filter&sa=D&source=editors&ust=1616072638060000&usg=AOvVaw1fw5RekZW44iGlJ662zseM), https://pubs.acs.org/doi/abs/10.1021/ac60214a047). 
The SG
filter is fit to a moving window of 11 data points where the center
...
regions of the spectra to 0 Absorbance. The higher the polynomial order,
...
"I-ModPolyfit" ([Zhao et al.
2007](https://www.google.com/url?q=https://paperpile.com/shared/BdMxHw&sa=D&source=editors&ust=1616072638062000&usg=AOvVaw1TxQwsOEQx3o-RAnIdLAsc)). This algorithm iteratively fits polynomial equations of the
...
(a) A Raman spectrum of polypropylene. Note that baseline increases
gradually toward the high Raman shift. (b) The baseline is fitted with a
sixth-order polynomial curve. (c) The baseline corrected spectrum of
polypropylene. Cowger et al. 2020. 
...
is an advanced feature proposed in [Renner et al.
2017](https://www.google.com/url?q=https://paperpile.com/shared/2nJXJF&sa=D&source=editors&ust=1616072638066000&usg=AOvVaw0_xARaOY_JGa7KMzmO9qp1) This can be a less intuitive approach but in cases where
...
Equation 2: Minimum Subtraction <!-- ![](images/image2.png) -->
...
[[https://www.compoundchem.com/2015/02/05/irspectroscopy/](https://www.google.com/url?q=https://www.compoundchem.com/2015/02/05/irspectroscopy/&sa=D&source=editors&ust=1616072638073000&usg=AOvVaw0OB5PjQTf3s2hyIcFnnT-i)
...
### How to differentiate between similar spectra? (Figures Courtesy of Bridget O\'Donnell from Horiba Scientific)

One common challenge is differentiating between LDPE and HDPE. But,
even with a low resolution instrument (MacroRAM, 2 cm-1/pixel), you can still see some differences. From a wide view,
...
bottom):
<!-- [![](images/image21.png)]{style="width: 584.61px"} -->
...
1. Download Spectragryph

[https://www.effemm2.de/spectragryph/down.html](https://www.google.com/url?q=https://www.effemm2.de/spectragryph/down.html&sa=D&source=editors&ust=1616072638079000&usg=AOvVaw24d2-Do_oAvlvpiJV7mpyL)
...
Cowger, W., Gray, A., Christiansen, S. H., De Frond, H., Deshpande, A.,
Hermabessiere, L., Lee, E., Mill, L., Munno, K., Ossmann, B., & Others.
(2020). EXPRESS: Critical Review of Processing and Classification
Techniques for Images and Spectra in Microplastic Research. Applied
Spectroscopy, 0003702820929064.

Chabuka, B. K., & Kalivas, J. H. (2020). Application of a Hybrid Fusion
...
Primpke, S., Wirth, M., Lorenz, C., & Gerdts, G. (2018). Reference
database design for the automated analysis of microplastic samples based
on Fourier transform infrared (FTIR) spectroscopy. Analytical and
Bioanalytical Chemistry.
[[https://doi.org/10.1007/s00216-018-1156-x](https://www.google.com/url?q=https://doi.org/10.1007/s00216-018-1156-x&sa=D&source=editors&ust=1616072638082000&usg=AOvVaw0TdyCl-hwArATY52caQaZF)
