...
[hyperSpec](https://cran.r-project.org/web/packages/hyperSpec/index.html). We
...
# There are some challenges with reproducing the names and floating point
# numbers but the underlying data is preserved. 
identical(unname(unlist(signif(as_OpenSpecy(hyperspecy)$spectra, 2))),
          unname(unlist(signif(scratch_OpenSpecy$spectra, 2))))
...
```{r, fig.align="center", out.width="98%"}
plotly_spec(scratch_OpenSpecy, rds_example)
```
...
If you have different wavenumber ranges for the spectra you want to combibne,
...
```{r}
# Extract the spectra with column names "8_5", "10_3", or "12_10" using a
# logical argument based on the spectra column names.
...
simplicity or you are running simulations or other proceedures that require you
...
peaks. After loading data, you can preprocess the data using intensity
