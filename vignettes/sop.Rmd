...
author: >
  Jessica Meyers, Jeremy Conkle, Win Cowger, Zacharias Steinmetz, Andrew Gray,
  Chelsea Rochman, Sebastian Primpke, Jennifer Lynch, Hannah Hapich,
  Hannah De Frond, Keenan Munno, Bridget O’Donnell
date: "`r Sys.Date()`"
output: 
  html_document: 
    toc: true
...
access drive with a [CC-BY 4.0 license](https://creativecommons.org/licenses/by/4.0/)
...
`wavenumber` and name the column with the intensities `intensity` (Table \@ref(sample-data))).

```{r sample-data, echo=FALSE}
knitr::kable(head(raman_hdpe), caption = "Table 1: Sample data `raman_hdpe`")
```
...
the analysis, intensity values are min-max normalized (Equation \@ref(eq:makerelative)).

\begin{equation}
\label{eq:makerelative}
...
<!-- Add Image for intensity dropdown -->
...
(SG)
filter](https://en.wikipedia.org/wiki/Savitzky%E2%80%93Golay_filter) (see also doi: 
...
"I-ModPolyfit" ([Zhao et al. 2007]()). This algorithm iteratively fits polynomial equations of the
