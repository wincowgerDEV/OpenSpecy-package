...
In the locally launched Shiny app, open **Advanced**, enable its master switch
and **Collapse Particle Spectra**, configure the signal/correlation and collapse
settings, then expand **Local H5 / ENVI source** and click **Open read-only**.
The app indexes the map and runs the region-sequential particle pipeline with
progress while spectra remain on disk. The Spectra box selects particle image,
thresholded, continuous signal, or correlation maps; map clicks show concise
metadata, and the threshold boxes replay the result histograms. The Thresholded
Particles archive can include any combination of the pipeline's tables,
objects, plots, and timing. Closing the source clears session state without
changing the source or completed cache generations. Browser uploads in local
and hosted modes are capped at 2 GB; an oversize selection opens a popup that
directs users to the local path workflow. The path opener is enabled by
`run_app()`, disabled by default in other deployments, and never enabled in
WebAssembly.
