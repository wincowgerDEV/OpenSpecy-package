...
The Shiny app deliberately uses a separate in-memory workflow. Its 10 GiB
upload ceiling is a transport limit rather than a promise that every operation
will fit in RAM. Before expensive operations, review the app's resident and
peak-memory estimate and reduce the data, reference library, PCA components, or
cluster count if the selected configuration is known to be unsafe.
