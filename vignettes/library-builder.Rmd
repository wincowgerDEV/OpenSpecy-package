...
call `join_lib_metadata()` directly when a custom join is needed. Automatic
`build_lib()` lookups coalesce curated lookup values back into existing
metadata columns, so lookup tables can correct fields such as `spectrum_type`.
