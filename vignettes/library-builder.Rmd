...
needed. Named `by` vectors map metadata names to different lookup names. An
explicit one-column lookup can set `fallback_by = "user_name"` to fill a blank
organization key before joining, and `fill_only = TRUE` to fill blank values
...
script is a straight-line composition of OpenSpecy functions. Canonically named
...
workflow runs. The official build merges a missing organization from
`user_name`, applies one source-type lookup, and asserts complete
library/spectrum-type coverage. The exact class lookup runs first;
