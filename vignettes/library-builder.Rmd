...
names. Known aliases are coalesced using an editable lookup table.
...
Add rows to `name_lookup` and pass it as `metadata_name_lookup` when a source
uses another name for the same metadata field. Ordinary and hierarchical joins
run whenever their corresponding lookup input is non-`NULL`.
