...
positions. Model fitting fills those restored gaps with the finite mean at each
wavenumber across training medoids. Each model stores that training-mean filler,
the support audit, per-lambda cross-validation metrics, and one tidy `tests`
table. Lambda is selected by maximum out-of-fold macro class accuracy; the
calibrated alpha, no-intercept, grouped multinomial, and inverse-frequency
weight settings are unchanged.
...
identification; production models are evaluated without retraining through
`match_spec()` so partial spectra can use the stored filler. Macro class
