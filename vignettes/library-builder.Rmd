...
workflow, an otherwise unresolved identity is labeled `other`; the build stops
for curation if `other` exceeds one percent of an artifact.
...
identities are retained under the curated generic-class policy. Derivative and
...
accuracy, per-class accuracy, confusion counts, and clear old/new
`assess_spec(report = "all")` shifts. Empty warnings retain their column schema
and numeric measurements remain numeric. The smaller 1,000-spectrum run in the
