...
Held-out groups are removed from full and medoid references before
identification. Logistic and random-forest assessments both fit a separate
model on only the source-local grouped training partition and predict its held
out groups; production models still use all eligible spectra.
...
`attr(reference_library_build$assessments, "evidence")`. The smaller 1,000-spectrum run in the
