...
ENVI and H5 maps are read into the compact in-memory `Specs` representation.
When S/N thresholding is enabled, the app records the selected metric, strict
bounds, smoothing choice, per-source S/N, and rejection reason; rejected pixels
map to an exact virtual zero spectrum. Turning thresholding off preserves every
pixel. **Compact Map (RDS)** downloads that complete transformed state.
Always keep a copy of the original files.

The app reads uploaded datasets into memory and has a 10 GiB total upload
ceiling. This ceiling only governs transport: preprocessing, PCA, clustering,
and identification also need working memory. The memory summary distinguishes
the estimated resident dataset from the estimated peak operation cost and
reports whether available RAM could be measured. If a known-unsafe operation is
blocked, try signal/noise-first particle collapse, fewer reference spectra,
fewer PCA components or clusters, a smaller input, or a machine with more RAM.
...
and defaults to 10. Matching works in bounded blocks and discards all other
correlations immediately. The Matches table and Top Matches download use this
same compact result, so neither view starts another identification pass.
