...
Ratio**. The sliders span the shared wavenumber axis in the uploaded data. Add
...
Calculations use a separate, non-derivative copy of the uploaded spectra, so
changing quantification settings does not change the spectra used for
identification or the displayed processing result. Draft names and sliders do
not start calculation; only saved definitions are calculated after
Quantification is enabled.

Choose one quantification treatment: **Fill Peaks baseline corrected
(recommended)**, **Modified polynomial baseline corrected**, **Min-max
normalized**, or **Uploaded spectrum (no quantification preprocessing)**. The
index formulas only define numerator and denominator bands; the selected
treatment is a separate preprocessing choice and affects comparability. Use the
same treatment when comparing index values between spectra.

The saved area- and peak-ratio definitions, values, and treatment are added to
the metadata in both **Processed Spectra** and **Top Matches** downloads.
...
the saved ratio definitions, values, and treatment are included in its
metadata.
