...
test_that("offline browser acceptance blocks all non-loopback requests", {
  ...
  expect_match(source, 'wavenumber\\[\\\\s,\\]\\+intensity')
  expect_match(source, 'mountedInput\\)\\.toBeEnabled')
  expect_match(source, 'captureDownload')
  expect_false(grepl('selectDownload\\(app, "Top Matches"\\)', source))
  expect_false(grepl("run_analysis", source, fixed = TRUE))
})
