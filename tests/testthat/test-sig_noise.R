...
test_that("sig_noise() can restrict the range correctly", {
   big_peak = sig_noise(raman_hdpe, metric = "sig", sig_max = 3000, sig_min = 2500)
   low_region = sig_noise(raman_hdpe, metric = "sig", sig_max = 1000, sig_min = 500)
   expect_true(big_peak > low_region)
   restric_peak <- restrict_range(raman_hdpe, min = 2500, max = 3000, make_rel = F) |>
