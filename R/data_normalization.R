...
#' >= 1, keeping absolute differences between values by shifting intensity values with the value of the smallest number.
...
#'
#' @details
#' \code{adjust_negative()} is used in Open Specy to avoid errors that could arise from log transforming spectra when using the \code{adjust_intensity()} and other functions.
#' \code {make_relative()} is used in Open Specy to retain the relative height proportions between spectra while avoiding the large numbers that can result from some spectral instruments. 
#'
...
#' \code{adjust_intensity()} for log transformation functions
#'
#' @examples
